...
There is one important fast path.  If every student has exactly the same rank
table, then exchanging two students never changes any of the five metrics.
The only information that matters is the count \(n_j\) assigned to each
laboratory.  In that special case, the program skips the student-laboratory
flow and optimizes the count vector directly: it fills lower-rank laboratories
before higher-rank ones, then maximizes the exact minimum fill rate, and only
then maximizes the exact average fill rate.  This is not a heuristic shortcut;
it solves the same objective on a smaller state space because all students are
interchangeable.
...
\section{Exact Minimum Fill-Rate Maximization}

After \(Q\), \(R_1\), and \(R_2\) have been fixed, the solver maximizes
\(U_{\min}\).  Every possible minimum fill-rate value has the form
\[
  q=\frac{a}{c_j}
\]
...
\begin{theorem}
The selected candidate maximizes \(U_{\min}\) without worsening \(Q\),
\(R_1\), or \(R_2\).
\end{theorem}
...
After \(Q\), \(R_1\), \(R_2\), and \(U_{\min}\) have been fixed, the solver
maximizes \(U_{\mathrm{avg}}\) without using a floating-point or scaled
...
\begin{theorem}
The final tie-break phase maximizes \(U_{\mathrm{avg}}\) without worsening
\(Q\), \(R_1\), \(R_2\), or \(U_{\min}\).
\end{theorem}

\begin{proof}
The final phase uses the already fixed rank threshold and the already fixed
minimum laboratory counts that realize the optimal \(U_{\min}\).  Therefore
every feasible flow in this phase preserves the earlier hard and
lexicographic constraints.  On a laboratory-to-sink edge, sending one more
unit to laboratory \(j\) decreases the min-cost objective by \(W_j\).  Hence
the fourth comparison component of a full flow is
\[
  -\sum_{j\in P} n_j W_j.
\]
...
\(U_{\mathrm{avg}}\).  Since the comparison of the weighted sums is performed
with integer arithmetic, no rounding can change the ordering.
\end{proof}

\subsection{Why Exact Comparison Is Needed Only at the End}

The average fill-rate comparison is not needed in every flow computation.
Before the final phase, the solver is still deciding \(Q\), then \(R_1\),
then \(R_2\), then \(U_{\min}\).  If two partial costs differ in any earlier
component, \(U_{\mathrm{avg}}\) cannot change the lexicographic decision.
Therefore the expensive big-integer comparison is used only in the final
tie-break phase, and only after the ordinary integer components are equal.

This preserves the same earlier optimum set:
\[
  \{x\mid Q,R_1,R_2,U_{\min}\ \text{are all optimal}\}.
\]
The final exact comparison only chooses the member of this set with maximum
\(U_{\mathrm{avg}}\).  Thus the exact version can change the assignment
relative to a scaled implementation only when the first four components tie
and the scaled approximation orders the remaining assignments incorrectly.
Such cases are possible with large capacities, which is why the submitted
solver uses exact big-integer comparison instead of claiming that a fixed
scale is always safe.
...
The implementation is a single C11 file, \texttt{assignment.c}.  It uses only
standard C library facilities.  Important implementation choices are:
\begin{itemize}[leftmargin=2em]
...
Seventh, the rubric objective uses monotonicity in the minimum-fill threshold
...
Seventh, if all students have identical rank rows, the program uses a closed
...
marginal value \(1/c_j\).  The order matters: average fill rate is used only
after the minimum fill rate has been fixed.  Finally, the count vector is
...
\midrule
\texttt{pdf\_sample} & Handout-format sample & General flow theorem \\
...
Long-name full preferences, 256 labs / 1024 students, rubric
  & 2.696s & 2.451s & 65.9MB input; read CPU 0.228s \\
...
Popular-skewed 128 labs / 512 students, exact counterfactual
  & 1.022s & 0.500s
  & selected solver CPU; counterfactual CPU 0.512s \\
\bottomrule
\end{tabular}
}
\end{center}
...
portfolio took 1.819 seconds serially and 0.543 seconds with
\texttt{--jobs 4}.  With \texttt{--portfolio-summary-only}, the same light
parallel portfolio took 0.548 seconds and removed per-candidate sidecars after
reading them.  The deep portfolio took 3.780 seconds serially and 1.416 seconds
...
The submitted solver does not enumerate assignments.  Instead, it uses the
total unimodularity and integrality of network flow to obtain exact assignment
solutions in polynomial time.  The resulting algorithm is an exact
Pareto-lexicographic flow solver for the specified objective order:
\[
