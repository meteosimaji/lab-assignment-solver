...
capacity-limited reward slots.  Each positive-capacity laboratory \(j\)
contributes \(g_j\) slots of reward \(W_j\), where \(g_j\) is its graph
capacity.  Every complete assignment selects exactly \(N\) such slots, so the
sum of the largest \(N\) slot rewards is an upper bound on \(U_s\), and the
sum of the smallest \(N\) slot rewards is a lower bound.  Their difference is
a valid \(D\) for the proof above and is no larger than the coarser
\(N(\max_j W_j-\min_j W_j)\) bound.
