...
exponential search.  With a succinct unrestricted objective, the model can
encode SAT: let each Boolean variable be represented by a student choosing one
of two laboratories, and let the objective evaluate a Boolean formula under
that induced truth assignment, scoring zero exactly on satisfying assignments.
A polynomial-time exact optimizer for every such succinct objective would
decide whether the optimum is zero and would therefore solve SAT in polynomial
time, implying \(P=NP\).  The implemented solver deliberately avoids this
claim by supporting only structured objective families with flow, threshold,
rational-comparison, or separable-convex structure.
