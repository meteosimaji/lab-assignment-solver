...
Seventh, repeated ungrouped min-cost-flow solves reuse templates when the
problem pointer, constraint pointer, and rank threshold are unchanged.  The
active-arc template records the student--laboratory arc list and adjacency
reservation counts.  The immutable graph template records source-to-student
and student-to-laboratory edges with placeholder costs.  Each solve deep-copies
that graph, patches the assignment-edge costs for the current objective, and
then appends the laboratory lower-bound and reward edges for the current count
vector.  Residual capacities and reverse-edge residual flow are therefore never
shared across solves, and objective-specific costs cannot leak from one
threshold check to another.
